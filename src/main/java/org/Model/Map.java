...
import org.Utils.ConquestMapReaderWriter;
import org.Utils.DefaultMapReaderWriter;
import org.Utils.LogLevel;
import org.Utils.MapReaderWriterAdapter;
...
public class Map implements Serializable {
    ...
    /**
     * Setter for the country list
     */
    public void setAllContinentsList(List<Continent> p_continents){ this.d_allContinentsAsList = p_continents; }

    /**
     * Setter for the Continent list
    */
    public void setAllCountriesList(List<Country> p_countries){ this.d_allCountriesAsList = p_countries; }
    ...
    public void removeContinent(String p_continentName) throws MapInvalidException{
        ...
        if(l_continentToBeRemoved != null){
            if(l_continentToBeRemoved.getCountries() == null){
                System.out.println("Continent " + p_continentName + " doesn't have any countries....!!!\nRemoving continent...");
            } else{
    ...
    public void loadMap(String p_fileName) throws MapInvalidException{
        try {
            ...
            l_scanner.close();
            //Read the map correctly based on the map type.
            if (l_fileLineByLine != null && !l_fileLineByLine.isEmpty()) {
                if(l_fileLineByLine.contains("[Territories]")) {
                    MapReaderWriterAdapter l_mapReaderAdapter = new MapReaderWriterAdapter(new ConquestMapReaderWriter());
                    l_mapReaderAdapter.readFromDefaultMapFile(l_map, l_fileLineByLine);
                }
                else if(l_fileLineByLine.contains("[countries]")){
                    new DefaultMapReaderWriter().readFromDefaultMapFile(l_map, l_fileLineByLine);
                }
                this.d_allCountriesAsList = l_map.getAllCountriesAsList();
                this.d_allContinentsAsList = l_map.getAllContinentsList();
            }
    ...
    public void saveMap(String p_fileName) throws MapInvalidException, IOException{
        ...
        if(saveMapValidityCheck(p_fileName)){
            if (this.validateMap()) {
                ...
                String l_userInput = "1";
                while(!l_userInput.equals("1") && !l_userInput.equals("2")) {
                    System.err.println("Invalid Input Passed.");
                    l_userInput = l_scanner.nextLine();
                }
                if (l_userInput.equalsIgnoreCase("1")) {
                    MapReaderWriterAdapter l_mapAdapter = new MapReaderWriterAdapter(new ConquestMapReaderWriter());
                    l_mapAdapter.writeToDefaultMapFile(p_fileName, this);
                }
                else if (l_userInput.equalsIgnoreCase("2")) {
                    new DefaultMapReaderWriter().writeToDefaultMapFile(p_fileName, this);
                }
