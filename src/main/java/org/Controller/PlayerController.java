...
import org.Model.PlayerStrategies.*;
...
public class PlayerController implements Serializable {
    ...
    public void addRemovePlayers(HashMap<String,ArrayList<String>> p_command){
        ...
        for(String p_name : l_allAddOperations){
            if(!this.checkIfPlayerAlreadyExist(p_name)){
                Player l_addNewPlayer = new Player(p_name);
                String l_playerStrategy = askForBehaviourStrategy(l_addNewPlayer);

                switch(l_playerStrategy) {
                    case "Human":
                        l_addNewPlayer.setPlayerStrategy(new HumanStrategy());
                        break;
                    case "Aggressive":
                        l_addNewPlayer.setPlayerStrategy(new AggressiveStrategy());
                        break;
                    case "Random":
                        l_addNewPlayer.setPlayerStrategy(new RandomStrategy());
                        break;
                    case "Benevolent":
                        l_addNewPlayer.setPlayerStrategy(new BenevolentStrategy());
                        break;
                    case "Cheater":
                        l_addNewPlayer.setPlayerStrategy(new CheaterStrategy());
                        break;
                    default:
                        System.out.println("Invalid Player Behaviour Provided!");
                ...
                d_allPlayers.add(l_addNewPlayer);
    ...
    private String askForBehaviourStrategy(Player p_addNewPlayer) {
        ...
        if(!AllTheConstants.PLAYER_BEHAVIORS.contains(l_playerStrategy)) {
            System.out.println("Invalid Behaviour Strategy Provided!");
            //this.setD_playerLog("Invalid Strategy Entered!");
            return askForBehaviourStrategy(p_addNewPlayer);
        }
    ...
    public Integer calculateArmiesForPlayer(Player p_player) {
        ...
        System.out.println("Generated Armies: " + p_player.getPlayerName() + " -> " + l_armies);
        return l_armies;
    }
